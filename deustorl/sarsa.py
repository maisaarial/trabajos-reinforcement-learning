...
class Sarsa:
    ...
    def learn(self, policy, n_steps:int=100, discount_rate=1, lr=0.01, lrdecay=1.0, n_episodes_decay=100, tb_episode_period=100, verbose=False):
        ...
        tblogger = TensorboardLogger("SARSA_(dr=" + str(discount_rate) +"-lr=" + str(lr) + "-lrdecay=" + str(lrdecay) + "e"+ str(n_episodes_decay)+ "-eps=" + str(current_epsilon) + ")" , episode_period=tb_episode_period)
